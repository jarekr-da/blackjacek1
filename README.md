...
```
daml build
daml codegen js .daml/dist/blackjacek-0.1.0.dar -o ui/daml.js
```
...
Deploying `blackjacek` to the hosted Daml platform
...
DAR `blackjacek.dar` you have just created.

To upload the UI, create a ZIP file containing all your UI assets by executing
```
daml build
daml codegen js .daml/dist/blackjacek-0.1.0.dar -o ui/daml.js
(cd ui && npm install && npm run-script build && zip -r ../blackjacek-ui.zip build)
```
...
(`blackjacek-ui.zip`) you have just created and publish it.

To see your deployed instance of `blackjacek` in action, follow the
...
- `daml.js/blackjacek-0.1.0/src/User.ts` (once you've generated it):
